...
@bot.command(name='d', help='Roll the arbitrary sided die because you want to for some reason.')
...
@bot.command(name='weather', help='returns weather info for a given zip code (try not to doxx yourself)')
async def weather(ctx, zip_code):
    ...
    response = "The temperature at " + str(zip_code) + " is " + str(
        int(weather_temp)) + " degrees fahrenheit, the weather conditions are described as " + weather_description
    await ctx.send(response)
...
async def stock(ctx, ticker):
    ...
    response = "Here's some information about that security you requested:\n" + "The current price is: " + \
               data["Global Quote"]["05. price"] + "\n" + "The current change is: " + data["Global Quote"][
                   "09. change"] + "\n" + "The change percent of that is: " + data["Global Quote"][
                   "10. change percent"] + "\n" + "The security opened at: " + data["Global Quote"][
...
async def pilot(ctx, characterName):
    char_srch = requests.get(f"https://esi.evetech.net/latest/search/?categories=corporation&datasource=tranquility"
...
async def corp(ctx, corporationName):
    ...
    corp_srch = requests.get(f"https://esi.evetech.net/latest/search/?categories=corporation&datasource=tranquility"
...
async def alice(ctx, alice_name):
    alice_srch = requests.get(f"https://esi.evetech.net/latest/search/?categories=alliance&datasource=tranquility"
    ...
    line4 = "**DOTLAN:** http://evemaps.dotlan.net/alliance/" + alice_id + "/" + "\n"
    response = line1 + line2 + line3 + line4

    await ctx.send(response)


@bot.command(name='shlookup', help='Deprecated, visit http://shlookup.sunkenrlyeh.com to use the shlookup!')
