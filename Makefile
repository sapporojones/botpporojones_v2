...
check-safety:
	...
	$(BANDIT_COMMAND_FLAG)poetry run bandit -ll -r botpporojones_v2/
...
check-style:
	...
	$(MYPY_COMMAND_FLAG)poetry run mypy --config-file setup.cfg botpporojones_v2 tests/**/*.py
...
# Example: make docker IMAGE=some_name VERSION=0.1.1
.PHONY: docker
docker:
...
# Example: make clean_docker IMAGE=some_name VERSION=0.1.1
.PHONY: clean_docker
clean_docker:
